...
firebase.auth().onAuthStateChanged((user) => {
	if (user) {
		...
		if (profileHeader) {
			db.collection('users').get().then(snapshot =>{
				snapshot.docs.forEach(doc =>{
					if (doc.data().userID === user.uid) {
						console.log(doc.data());
						const usernameHeaderProf = document.getElementById('usernameHeader');
		...
		if (aestheticInteractionDiv) {
			...
			console.log('currentuser' + currentUser.innerHTML.toLowerCase(), 'Owner' + owner.toLowerCase());
			console.log(owner);
				...
				snapshot.docs.forEach(doc => {
					...
					if (doc.data().username === owner) {
						...
						db.collection('users').doc(ownerUID).collection('aesthetics').get().then(snapshot => {
							snapshot.docs.forEach(doc => {
								if (doc.data().aestheticName === aestheticLookup) {
									console.log(doc.data());
		...
		db.collection('users').get().then(snapshot => {
			snapshot.docs.forEach(doc =>{
				if (doc.data().username.toLowerCase() === usernameHeader.innerHTML.toLowerCase()) {
					...
					if (usernameHeader.innerHTML.toLowerCase() === currentuser.innerHTML.toLowerCase()) {
						unfollowButton.style.display = 'none';
						followBtn.style.display = 'none';
					}
					if (usernameHeader.innerHTML.toLowerCase() !== currentuser.innerHTML.toLowerCase()) {
						...
						let i;
						for (i = 0; i < doc.data().followersArray.length; i++) {
							console.log(doc.data().followersArray[i]);
							if (doc.data().followersArray[i] === currentuser.innerHTML.toLowerCase()) {
								followBtn.style.display = 'none';
								unfollowButton.style.display = 'block';
							} else {
								unfollowButton.style.display = 'none';
...
// todo fix not updating the number of followers
		...
		db.collection('users').get().then(snapshot => {                                                   // cycle through documents in the 'users' database
			snapshot.docs.forEach(doc => {
				...
				let lowerUsername = usernameHeader.innerHTML.toLocaleLowerCase();
